...
export const PendulumSimulator = () => {
  ...
  const [accelerationData, setAccelerationData] = useState<
    { time: number; acceleration: number }[]
  >([]); // Almacena los datos de posición
    ...
    const updatePosition = () => {
      ...
      if (currentTime - lastUpdateTime >= 0.05) {
        ...
        const newVelocityData = [
          ...velocityData,
          { time: currentTime, velocity: velocity },
        ];
        ...
        const newAccelerationData = [
          ...accelerationData,
          { time: currentTime, acceleration: acceleration },
        ];
  ...
  const handleMouseMove = (e: any) => {
    if (dragging) {
      ...
      const clampedAmplitude = Math.min(
        Math.max(newAmplitude, minAmplitude),
        maxAmplitude,
      );
  ...
  const handleShowPositionChart = () => {
    ...
    setIsAnyChartVisible(
      !showPositionChart || showVelocityChart || showAccelerationChart,
    );
  };
  ...
  const initializeAccelerationChart = () => {
    const accelerationCanvas = document.getElementById(
      "acceleration-chart",
